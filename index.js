...
function is31Flavors(flavorList) {
    return flavorList.length === 31 ? true : false;
}
...
function addFlavor(flavorList, newFlavor) {
    return [newFlavor, ...flavorList]
}
console.log(addFlavor([2, 3, 4], 1))
...
function removeFlavorByName(flavorList, flavorToRemove) {
    return flavorList.filter(flavor => flavor !== flavorToRemove);
}
...
function filterByWord(flavorList, filterFlavor) {
    const regex = new RegExp(filterFlavor);
    return flavorList.filter(flavor => flavor.match(regex))
}
...
function getAverageWordLength(wordArray) {
    ...
    const avgLength = wordLengthArray.reduce((a, b) => a + b) / wordArray.length;
    return avgLength;
}
